...
export function isMinAndInvalid<T extends Record<string, any>>(value: number, obj: T, key: Key<T>, control: AbstractControl): boolean {
  ...
  if (condition) {
    setError(control, {type: 'MIN_ERROR', params: {minValue: minValidation.min}});
    return true;
  }
